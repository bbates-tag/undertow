...
- **The simulator sets the floor, not the ceiling.** `npm run sim` plays runs with a deliberately mediocre greedy bot (no tide timing, no charge banking, mechanical drafting — though it does resolve conditional riders like Flood/Read against live state since the Wakeweaver landed). Current envelope: Tidecaller ~13%, Wakeweaver ~3%, floors ~20–30 of 36; compare characters only against the same bot version. A human who reads intents and times the tide should sit around 15–30% at Depth 0, trending toward the classic roguelite "earned win." Depth 1–10 exist to keep 90%+ winrate players uncomfortable.
